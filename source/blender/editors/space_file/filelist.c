...
static bool filelist_need_filtering(struct FileList *filelist)
{
  return ((filelist->flags & FL_NEED_FILTERING) ||
          (filelist->ae && (filelist->ae->flag & AE_DIRTY_FILTER)));
}
...
static void filelist_cache_previews_push(FileList *filelist, FileDirEntry *entry, const int index)
{
  ...
  const bool do_preview = ((filelist->ae && filelist->ae->type->previews_get != NULL) ||
                           (entry->typeflag & (FILE_TYPE_IMAGE | FILE_TYPE_MOVIE |
                                               FILE_TYPE_FTFONT | FILE_TYPE_BLENDER |
                                               FILE_TYPE_BLENDER_BACKUP | FILE_TYPE_BLENDERLIB)));
...
{
  if (sfile->files ==
      NULL) { /* Can happen at file opening if a filebrowser editor is opened in file. */
    return NULL;
  }
  ...
  if (filelist->ae) {
    ...
    for (i = 0, entry = tmp_arr.entries.first; i < size && entry; i++, entry = entry->next) {
      ...
      if (!entry->name) {
        ...
        entry->name = BLI_strdup(
            fileentry_uiname(filelist->filelist.root, entry->relpath, entry->typeflag, buff));
      }
      if (!entry->entry) {
...
bool filelist_cache_previews_update(FileList *filelist)
{
  ...
  if (filelist->ae) {
    ...
    cache->ae_preview_job = previews_get(
        filelist->ae, cache->ae_preview_job, &cache->ae_preview_uuids);
                                        ...
                                        const bool use_ae)
{
  ...
  for (; !BLI_ghashIterator_done(iter); BLI_ghashIterator_step(iter)) {
    ...
    if (entry_org && (((ELEM(check, CHECK_ALL, CHECK_NONE))) ||
