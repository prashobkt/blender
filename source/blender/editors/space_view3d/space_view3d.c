...
static bool view3d_path_link_append_drop_poll(bContext *C,
                                              wmDrag *drag,
                                              const wmEvent *event,
                                              ...
                                              const bool is_link)
{
  if ((!event->shift && !is_link) || (event->shift && is_link)) {
    if (drag->type == WM_DRAG_LIBRARY) {
      ...
      char *group, *name;
      if (!BLO_library_path_explode(drag_data->path,
                                    libname,
                                    &group,
...
static bool view3d_path_link_drop_poll(bContext *C,
                                       wmDrag *drag,
                                       const wmEvent *event,
                                       const char **tooltip)
{
...
static bool view3d_path_append_drop_poll(bContext *C,
                                         wmDrag *drag,
                                         const wmEvent *event,
                                         const char **tooltip)
{
...
static void view3d_path_link_drop_copy(wmDrag *drag, wmDropBox *drop)
{
  ...
  RNA_string_set(drop->ptr,
                 "filepath",
                 drag_data->path); /* Needed only to avoid invoke to open filebrowser. */
